...
static void tegra_machine_restart(char mode, const char *cmd)
{
#if 0
	//for OTA mode,+++
	//set "boot-recovery" to partition of MSC
	//bootloader will check MSC to auto enter OTA mode
	int fd = -1;
	char cmdRecovery[] = {"hello"}; //{"boot-recovery"};
	int ret = 0;
	int pos = 0;
	char command[32];
	...
	if( cmd != NULL )
	{
		printk("kernel common=====>restart from %s \n",cmd);
		
		if( strcmp(cmd, "factory-reset") == 0  )
		{
			printk("kernel common=====>enter recovery mode\n");
			...
			if( fd > 0 )
			{
				...
				//printk("recovery mode======> BootMessage.command = %s\n",BootMessage.command);

				//just write BootMessage.command into MSC
				pos = sys_lseek(fd, SECTORSIZE*MISC_COMMAND_PAGE, SEEK_SET );			
				ret = sys_write(fd, command, sizeof(command) );
				if(ret <= 0) 
				{	        	                
					printk("kernel common======> msc write error\n");
				}
				else
					printk("kernel common======> msc write ok\n");			
			...
			{
				printk("kernel common======> msc file open error\n");
			}
			set_fs(old_fs);

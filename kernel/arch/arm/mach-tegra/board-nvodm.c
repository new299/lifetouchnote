...
# define BT_DEPRECATED_RESET 2
# define BT_WAKEUP 3
# define BT_DEPRECATED_WAKEUP 4
...
#endif
/* YC+++ for thermal sensor */
#if defined(CONFIG_HWMON) && defined(CONFIG_SENSORS_TEGRA)
static struct platform_device tegra_thermal_device = 
{
    .name = "tegra_thermal",
    .id   = -1,
};
...
static noinline void __init tegra_setup_bluesleep(void)
{
	...
	res[1].start  = TEGRA_GPIO_PH6;
	res[1].end    = TEGRA_GPIO_PH6;
...
#if 1
static struct ltna7_gps_platform_data ltna7_gps_platform;
static struct platform_device ltna7_gps_device = {
	.name = "ltna7-gps-rfkill",
	.dev = {
		.platform_data = &ltna7_gps_platform,
	},
};
#endif
...
static noinline void __init tegra_setup_rfkill(void)
{
	...
	lbee9qmb_platform.gpio_oldrst=-1;
	lbee9qmb_platform.gpio_btwake=-1;
	lbee9qmb_platform.gpio_oldbtwake=-1;
	...
	else if ((con = NvOdmPeripheralGetGuid(NV_ODM_GUID('b','c','m','_','4','3','2','9'))))
	{
		...
		for (i=0; i<con->NumAddress; i++) {
					...
					lbee9qmb_platform.gpio_pwr = nr_gpio;
						...
						&& con->AddressList[i].Purpose == BT_DEPRECATED_RESET ){
					nr_gpio = con->AddressList[i].Instance * 8 +
						 con->AddressList[i].Address;
					lbee9qmb_platform.gpio_oldrst = nr_gpio;
				}
#ifndef CONFIG_BT_BLUESLEEP
			else if (con->AddressList[i].Interface == NvOdmIoModule_Gpio
						&& con->AddressList[i].Purpose == BT_WAKEUP ){
					nr_gpio = con->AddressList[i].Instance * 8 +
						 con->AddressList[i].Address;
					lbee9qmb_platform.gpio_btwake = nr_gpio;
				}
			else if (con->AddressList[i].Interface == NvOdmIoModule_Gpio
						&& con->AddressList[i].Purpose == BT_DEPRECATED_WAKEUP){
					nr_gpio = con->AddressList[i].Instance * 8 +
						 con->AddressList[i].Address;
					lbee9qmb_platform.gpio_oldbtwake = nr_gpio;
				}
#endif
		}
	...
	if ((con = NvOdmPeripheralGetGuid(NV_ODM_GUID('a','s','a','r','i','g','p','s'))))
	{
		int nr_gpio;
		for (i=0; i<con->NumAddress; i++) {
			if (con->AddressList[i].Interface == NvOdmIoModule_Gpio) {
				nr_gpio = con->AddressList[i].Instance * 8 +
				                   con->AddressList[i].Address;
				ltna7_gps_platform.gpio_pwr = nr_gpio;
			}
		}
		ltna7_gps_platform.delay=5;
		if (platform_device_register(&ltna7_gps_device))
			pr_err("%s: registration failed\n", __func__);
	}
...
//add by 
#ifdef CONFIG_LEDS_NVEC_PWM
//data
static struct gpio_led nvec_leds_datas[] = {
	{	// green
		.name			= "green",
		//.gpio			= AT91_PIN_PB2,
		//.active_low		= 1,
		.default_trigger	= "led-green-trigger",
	},
    /*
	{	// "blue"
		.name			= "blue",
		//.gpio			= AT91_PIN_PB1,
		//.active_low		= 1,
		.default_trigger	= "led-blue-trigger",
	},
    */
};
...
static struct platform_device nvec_leds_device = {
	.name		= "nvec_leds",
	.id			= -1,
	.dev.platform_data	= &led_data,
};

void __init nvec_leds_register(struct gpio_led *leds, int nr)
{
	if (!nr)
		return;
	...
	platform_device_register(&nvec_leds_device);
}
#endif

#ifdef CONFIG_SWITCH_HEADSET
static struct gpio_switch_platform_data tegra_headset_data =
{
    .name = "h2w",
    .gpio = 'w'-'a',
    //.pin = 2,
    //.name_on = "aaa",
    //.name_off = "bbb",
    .state_on = "Headset insert",
    .state_off = "Headset remove",
};

static struct platform_device tegra_headset_device = 
{
    .name = "headset-switch",
    .id   = -1,
    .dev =
    {
        .platform_data = &tegra_headset_data,
    },
};
#endif

#ifdef CONFIG_QCI_VIBRATOR
static struct platform_device qci_vibrator_device =
{
    .name = "tegra_vibe",
    .id   = -1,
};
...
#ifdef CONFIG_QCI_MSENSOR
static struct platform_device qci_Msensor_device =
{
    .name = "tegra_Msensor",
    .id   = -1,
};
...
static struct i2c_board_info pmu_info[] __initdata = {
};
...
static void tegra_board_setup_i2c_dev(void)
{
	i2c_register_board_info(0, i2c_devs0, ARRAY_SIZE(i2c_devs0));
	i2c_register_board_info(1, i2c_devs1, ARRAY_SIZE(i2c_devs1));
	i2c_register_board_info(2, i2c_devs2, ARRAY_SIZE(i2c_devs2));
	i2c_register_board_info(3, pmu_info, ARRAY_SIZE(pmu_info));
}
#else
...
//add by 
#ifdef CONFIG_LEDS_NVEC_PWM
    nvec_leds_register(nvec_leds_datas, ARRAY_SIZE(nvec_leds_datas));  
#endif

#ifdef CONFIG_SWITCH_HEADSET
    (void) platform_device_register(&tegra_headset_device);
#endif

#ifdef CONFIG_QCI_VIBRATOR
    (void) platform_device_register(&qci_vibrator_device);
#endif

#ifdef CONFIG_QCI_MSENSOR
	(void) platform_device_register(&qci_Msensor_device);
#endif
//end 

/* YC +++ for thermal sensor */
#if defined(CONFIG_HWMON) && defined(CONFIG_SENSORS_TEGRA)
    (void) platform_device_register(&tegra_thermal_device);
#endif
/* YC +++ end */
	return 0;
}

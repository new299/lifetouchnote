...
static void debug_gpio(int gpio)
{
...
#define GPIO_INT_CLR(x)               (GPIO_REG(x) + 0x70)
        ...
        printk("gpio: [%d] int:%d enb:%d\n",
               gpio,
               (__raw_readl(GPIO_INT_STA(gpio)) >> GPIO_BIT(gpio)) & 1,
               (__raw_readl(GPIO_INT_ENB(gpio)) >> GPIO_BIT(gpio)) & 1);
}
...
static int tegra_suspend_enter(suspend_state_t state)
{
    ...
    for_each_irq_desc(irq, desc) {
        ...
        /* ADHOC: special treat for interrupt from EC */
        if (gpio_get_value(0) == 0) {
                printk("======= pass suspending =======\n");
                debug_gpio(0);
                goto pass;
        }

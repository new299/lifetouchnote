...
static int fsl_udc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (udc_controller->vbus_active)
	{
		...
		reset_queues(udc_controller);
                udc_controller->vbus_active_before_suspend = udc_controller->vbus_active;
		udc_controller->vbus_active = 0;
...
static int fsl_udc_resume(struct platform_device *pdev)
{
	...
	if (!(fsl_readl(&usb_sys_regs->vbus_wakeup) & USB_SYS_VBUS_STATUS))
        ...
        /* update vbus_active */
        if (udc_controller->vbus_active_before_suspend) {
                u32 val = fsl_readl(&usb_sys_regs->vbus_wakeup) & USB_SYS_VBUS_STATUS;
                udc_controller->vbus_active = val ? 1 : 0;
                printk(KERN_INFO "%s: vbus_active:%d\n",
                       __FUNCTION__, udc_controller->vbus_active);
        }

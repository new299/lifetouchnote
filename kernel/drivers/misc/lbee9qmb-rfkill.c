...
{
	struct lbee9qmb_platform_data *plat = pdev->dev.platform_data;
	if (plat->gpio_btwake!=-1 && plat->curr_state)
		gpio_set_value(plat->gpio_btwake, 1);
	if (plat->gpio_oldbtwake!=-1 && plat->curr_state)
		gpio_set_value(plat->gpio_oldbtwake, 1);
	return 0;
}
